...
func createPart(p *part) *InterceptRequest {
	id := "first"
	if p != nil && p.id != "" {
		id = p.id
	}
...
func runIntercept(i *Interceptor, req *InterceptRequest, res *InterceptResult, wg *sync.WaitGroup) {
	go func() {
		*res = *i.Intercept(req)
		wg.Done()
	}()
...
func Test_NotBought_SinglePart(t *testing.T) {
	...
	assert.Equal(t, INTERCEPT_CANNOT_HANDLE, res.Action)
	assertEmpty(t, i)
}
...
func Test_NotBought_TwoParts(t *testing.T) {
	...
	wg.Add(2)
	var res1 InterceptResult
	runIntercept(i, createPart(&part{id: "first", scid: 999}), &res1, &wg)

	var res2 InterceptResult
	runIntercept(i, createPart(&part{id: "second", scid: 999}), &res2, &wg)
	wg.Wait()
	assert.Equal(t, INTERCEPT_CANNOT_HANDLE, res1.Action)
	assert.Equal(t, INTERCEPT_CANNOT_HANDLE, res2.Action)
	assertEmpty(t, i)
}
...
func Test_NoMpp_Happyflow(t *testing.T) {
	...
	assert.Equal(t, INTERCEPT_RESUME_WITH_ONION, res.Action)
...
func Test_NoMpp_AmtBelowMinimum(t *testing.T) {
	...
	assert.Equal(t, FAILURE_UNKNOWN_NEXT_PEER, res.FailureCode)
	assertEmpty(t, i)
}
...
func Test_NoMpp_CltvDeltaBelowMinimum(t *testing.T) {
	...
	assert.Equal(t, INTERCEPT_FAIL_HTLC_WITH_CODE, res.Action)
	assert.Equal(t, FAILURE_INCORRECT_CLTV_EXPIRY, res.FailureCode)
	assertEmpty(t, i)
}
...
func Test_Mpp_SinglePart_AmtTooSmall(t *testing.T) {
	...
	assert.Equal(t, INTERCEPT_FAIL_HTLC_WITH_CODE, res.Action)
	assert.Equal(t, FAILURE_TEMPORARY_CHANNEL_FAILURE, res.FailureCode)
...
func Test_Mpp_TwoParts_FinalizedOnSecond(t *testing.T) {
	...
	wg.Wait()
	assert.Equal(t, INTERCEPT_RESUME_WITH_ONION, res1.Action)
	...
	assert.Equal(t, INTERCEPT_RESUME_WITH_ONION, res2.Action)
...
func Test_Mpp_BadSecondPart_ThirdPartCompletes(t *testing.T) {
	...
	assert.Equal(t, INTERCEPT_FAIL_HTLC_WITH_CODE, res2.Action)
	assert.Equal(t, FAILURE_AMOUNT_BELOW_MINIMUM, res2.FailureCode)

	assert.Equal(t, INTERCEPT_RESUME_WITH_ONION, res3.Action)
...
func Test_Mpp_PartReplacement(t *testing.T) {
	...
	wg.Wait()
	assert.Equal(t, INTERCEPT_IGNORE, res1.Action)
